...
def load_users_from_file():
    ...
    users_file = os.path.join(script_dir, "users_list.txt")
    user_ids = set()
    try:
...
def main():
    ...
    print(f"--- Мониторинг света ДТЭК (Черга {config.MY_QUEUE}) ---")

    # Отключаем SSL-предупреждения (как CURLOPT_SSL_VERIFYPEER => false в PHP)
    requests.packages.urllib3.disable_warnings(requests.packages.urllib3.exceptions.InsecureRequestWarning)

    session = requests.Session()
    schedule_data = get_dtek_schedule(session)
    if not schedule_data:
        print("DEBUG: Не удалось получить данные ДТЭК. Выход.")
        return

    print("DEBUG: Парсинг полного графика...")
    full_schedule = parse_full_schedule(schedule_data)
    if not full_schedule:
        print("DEBUG: Не удалось разобрать полный график. Выход.")
        # Все равно сохраняем пустой файл, чтобы показать, что скрипт отработал
        save_schedule_to_txt({})
        return

    # Сохраняем результат в svet.txt (как ты просил)
    save_schedule_to_txt(full_schedule)

    # --- НОВАЯ ЛОГИКА: Проверка текущего статуса ---
    current_power_on = is_power_on_at_current_time(full_schedule) # Новая функция
    print(f"DEBUG: Свет включен в текущий момент: {current_power_on}")

    if not current_power_on:
         print("DEBUG: Свет в данный момент *не включен*. Пропуск поиска ближайшего отключения.")
         # Все равно сохраняем график
         return # Если света нет, не нужно оповещать о будущем отключении

    # --- СТАРАЯ ЛОГИКА: Поиск ближайшего отключения ---
    outage_info = find_imminent_outage(full_schedule)
    if not outage_info:
        print("DEBUG: Ближайшее отключение не найдено или уже прошло. Выход.")
        return

    cut_off_start_hour = outage_info['cut_off_start_hour']
    cut_off_start_minute = outage_info['cut_off_start_minute']
    expected_on_hour = outage_info['expected_on_hour']
    expected_on_minute = outage_info['expected_on_minute']
    minutes_until_cut_off = int(outage_info['minutes_until_cut_off'])
    status = outage_info['status']

    # Расшифровка статуса для Telegram
    if status in ['no', 'first', 'second']:
        status_text = "🔴 *Гарантоване відключення*"
    elif status in ['maybe', 'mfirst', 'msecond']:
        status_text = "🟡 *Можливе відключення*"
    else:
        status_text = f"⚪️ *{status}*"

    print(f"DEBUG: Ближайшее отключение ({status_text}) найдено с {cut_off_start_hour:02d}:{cut_off_start_minute:02d} до {expected_on_hour:02d}:{expected_on_minute:02d}. Осталось {minutes_until_cut_off} мин.")

    # Проверка критического времени и отправка оповещения
    if 0 < minutes_until_cut_off <= CRITICAL_TIME_MINUTES:
        # Определяем тип отключения для уведомления
        off_time = f"{cut_off_start_hour:02d}:{cut_off_start_minute:02d} - "
        # Если свет включается в полночь (24:00), пишем "24:00"
        if expected_on_hour == 24 and expected_on_minute == 0:
            on_time = "24:00"
            duration_text = "до кінця доби"
        else:
            on_time = f"{expected_on_hour:02d}:{expected_on_minute:02d}"
            # Просто вычисляем продолжительность для уведомления
            start_total_mins = cut_off_start_hour * 60 + cut_off_start_minute
            # Обрабатываем случай 24:00
            if expected_on_hour == 24 and expected_on_minute == 0:
                # 24:00 = 24 * 60 = 1440 минут
                end_total_mins = 24 * 60
            else:
                end_total_mins = expected_on_hour * 60 + expected_on_minute
            # Корректируем на случай перехода через полночь (редко, но если логика выше позволяет)
            # В текущем контексте переход через полночь маловероятен, так как мы анализируем один день
            if end_total_mins <= start_total_mins:
                 duration_text = "до кінця доби"
            else:
                duration_mins = end_total_mins - start_total_mins
                duration_text = f"на {duration_mins} хвилин"

        message = f"⚡️ **УВАГА: ОЧІКУЄТЬСЯ ВІДКЛЮЧЕННЯ (ДТЕК)!**\n"
        message += f"🏠 **Адреса:** {config.STREET}, буд. {config.HOUSE}\n"
        message += f"👥 **Черга:** {config.MY_QUEUE}\n"
        message += f"🔌 **Відключення:** {off_time}{on_time} ({duration_text})\n"
        message += f"💡 **Можливе включення:** *{on_time}*\n"
        message += f"📊 **Статус:** {status_text}"

        print(f"INFO: Время критическое ({minutes_until_cut_off} мин.) И Свет включен. Отправляю оповещение.")
        send_telegram_message(message)
    else:
        print("DEBUG: До отключения больше 30 минут. Оповещение не требуется.")
